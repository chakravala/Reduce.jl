...
function show_expr(io::IO, expr::Expr) # recursively unparse Julia expression
  ...
  elseif expr.head == :(=)
    show_expr(io,expr.args[1]); print(io,":="); show_expr(io,expr.args[2])
  elseif expr.head == :for
    print(io,"for "); show_expr(io,expr.args[1]); show_expr(io,expr.args[2])
  elseif expr.head == :block
    print(io,"begin "); show_expr(io,expr.args[1]); for k ∈ 2:length(expr.args[2:end])+1
      print(io,";"); show_expr(io,expr.args[k]); end; print(io," end")
  elseif expr.head == :function; print(io,"procedure ")
    show_expr(io,expr.args[1]); print(io,";"); show_expr(io,expr.args[2])
  elseif expr.head == :return
    print(io,"return "); show_expr(io,expr.args[1])
  elseif expr.head == :macrocall
    if expr.args[1] == Symbol("@big_str"); print(io,expr.args[2])
    else; error("Macro $(expr.args[1]) block structure not implemented") end
  elseif expr.head == :(:)
    show_expr(io,expr.args[1]); print(io,":"); show_expr(io,expr.args[2]); print(io," ")
...
function unparse(expr::Expr); str = Array{Compat.String,1}(0); io = IOBuffer()
  if expr.head == :block; for line ∈ expr.args # block structure
      show_expr(io,line); push!(str,String(take!(io))); end
    str[1] = "begin "*str[1]; str[end] = str[end]*" end"; return rtrim(str)
...
function rtrim(r::Array{Compat.String,1}); n = Array{Compat.String,1}(0)
  for h ∈ 1:length(r); !isempty(r[h]) && push!(n,r[h]); end; return n; end
function split(r::RExpr); n = Array{Compat.String,1}(0)
  for h ∈ 1:length(r.str); p = split(replace(r.str[h],r"\$",";"),';')
    for t ∈ 1:length(p); push!(n,p[t]); end; end; return RExpr(rtrim(n)); end
...
function parse(r::RExpr,be=0)
  ...
  while !done(iter,state); (h,state) = next(iter, state)
    ...
    elseif contains(sexpr[h],":")
      ...
      js=sexpr[h]; se=sum(sh.=="end"); 0<se≤be ? (js=replace(js,"end","")) :
